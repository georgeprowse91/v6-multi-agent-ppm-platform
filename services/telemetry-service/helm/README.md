...
## Purpose
This chart packages the **telemetry-service** deployment for the Multi-Agent PPM platform when deploying to
Kubernetes. It defines the Deployment, Service, and ConfigMap used by the telemetry-service runtime.

## Responsibilities
- Package the telemetry-service container image and runtime settings.
...
## Folder structure
```
services/telemetry-service/helm/
├── Chart.yaml
├── values.yaml
├── templates/
│   ├── _helpers.tpl
...
## Conventions
- `Chart.yaml` **name** must match the parent directory name (`telemetry-service`).
...
1. Add a new manifest under `services/telemetry-service/helm/templates/` (e.g. `ingress.yaml`).
