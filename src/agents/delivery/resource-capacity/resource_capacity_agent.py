...
class ResourceCapacityAgent(BaseAgent):
    ...
    def __init__(
        ...
        self.max_allocation_threshold = config.get("max_allocation_threshold", 1.0) if config else 1.0
        self.skill_matching_threshold = config.get("skill_matching_threshold", 0.70) if config else 0.70
    ...
    async def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action == "add_resource":
            ...
            return await self._match_skills(
                input_data.get("skills_required", []),
    ...
    async def _match_skills(
        ...
        for resource_id, resource in self.resource_pool.items():
            ...
            match_score = await self._calculate_skill_match_score(
                resource_skills,
                required_skills
            )
    ...
    async def _forecast_capacity(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        future_capacity = await self._forecast_future_capacity(
            self.forecast_horizon_months
        )

        # Forecast future demand (from project pipeline)
        future_demand = await self._forecast_future_demand(
            self.forecast_horizon_months
        )

        # Identify bottlenecks
        bottlenecks = await self._identify_capacity_bottlenecks(
            future_capacity,
            future_demand
        )
    ...
    async def _get_availability(
        ...
        return {
            ...
            "average_availability": sum(
                d.get("available_hours", 0) for d in availability_by_day
            ) / len(availability_by_day) if availability_by_day else 0
        }
    ...
    async def _get_utilization(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        average_utilization = sum(
            u["utilization"] for u in utilization_data
            ...
            for i, alloc1 in enumerate(resource_allocations):
                for alloc2 in resource_allocations[i+1:]:
                    ...
                    if overlap.get("has_overlap"):
                        ...
                        total_allocation = (
                            alloc1.get("allocation_percentage", 0) +
                        ...
                        if total_allocation > 100:
                            conflicts.append({
                                "resource_id": resource_id,
                                "resource_name": self.resource_pool.get(resource_id, {}).get("name"),
                                "allocation_1": alloc1,
                                "allocation_2": alloc2,
    ...
    async def _forecast_future_capacity(self, months: int) -> List[Dict[str, Any]]:
        ...
        current_capacity = await self._calculate_total_capacity()
        return [
            {"month": i, "capacity": current_capacity}
    ...
    async def _create_baseline_scenario(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "utilization": await self._calculate_total_demand() / await self._calculate_total_capacity()
        }
