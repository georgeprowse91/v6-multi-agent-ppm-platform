...
@pytest.mark.asyncio
async def test_correlation_id_propagates_logs_metrics_and_results(
    ...
    monkeypatch.setattr(
        "agents.runtime.src.base_agent.build_agent_execution_metrics",
        lambda _name: _ExecutionMetrics(),
    )
    monkeypatch.setattr(
        "agents.runtime.src.orchestrator.build_agent_execution_metrics",
        lambda _name: _ExecutionMetrics(),
    )
    ...
    result = await orchestrator.run(
        tasks, context={"tenant_id": "t1", "correlation_id": correlation_id}
    )
...
@pytest.mark.asyncio
async def test_correlation_id_present_in_base_agent_logs(
    monkeypatch: pytest.MonkeyPatch, caplog: pytest.LogCaptureFixture
) -> None:
    monkeypatch.setattr(
        "agents.runtime.src.base_agent.build_cost_metrics", lambda _name: _CostMetrics()
    ...
    agent_records = [
        record for record in caplog.records if getattr(record, "correlation_id", None) == corr_id
    ]
