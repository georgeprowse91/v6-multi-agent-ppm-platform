...
- `apps/analytics-service/helm/templates`: Templates used by the component (deployment or message content).
- `apps/analytics-service/helm/Chart.yaml`: Helm chart metadata and versioning.
- `apps/analytics-service/helm/values.yaml`: Helm values for environment-specific overrides.
...
```bash
helm lint apps/analytics-service/helm
helm template analytics-service apps/analytics-service/helm
```
