...
def _assistant_context(
    tenant_id: str, project_id: str | None, correlation_id: str
) -> dict[str, Any]:
    return {"tenant_id": tenant_id, "project_id": project_id, "correlation_id": correlation_id}


def _fallback_suggestions(
    payload: AssistantSuggestionRequest, state: Any, methodology_map: dict[str, Any]
    ...
    if payload.incomplete_prerequisites:
        for prereq in payload.incomplete_prerequisites[:3]:
            suggestions.append(
                AssistantSuggestion(
                    id=f"sug-prereq-{prereq.activity_id}",
                    label=f"Complete prerequisite: {prereq.activity_name}",
                    category="prerequisite",
                    priority="high",
                    action_type="navigate",
                    payload={"target_activity_id": prereq.activity_id, "stage_id": prereq.stage_id},
                )
            )
    if not suggestions:
        suggestions.append(
    ...
    user_prompt = json.dumps(
        {
            ...
            "incomplete_prerequisites": [p.model_dump() for p in payload.incomplete_prerequisites],
        }
    )
...
async def assistant_query(
    ...
    if "risk" in lowered:
        ...
        items = [
            f"{i.get('project', 'Unknown')} \u2014 {i.get('risk', 'Risk')} ({i.get('title', '')})"
            for i in approvals
            if i.get("project")
        ]
        return AssistantQueryResponse(
            query=query_text, summary="Projects with elevated risk exposure:", items=items
        )
    if "workflow" in lowered or "automation" in lowered:
        ...
        return AssistantQueryResponse(
            query=query_text, summary="Workflow health summary:", items=items
        )
    if "approval" in lowered:
        approvals = _approval_payload().get("items", [])
        items = [i.get("title", "Approval review") for i in approvals]
        return AssistantQueryResponse(
            query=query_text, summary="Pending approvals to review:", items=items
        )
...
async def assistant_demo_conversation(scenario: str, request: Request) -> DemoConversationResponse:
    ...
    return DemoConversationResponse(
        scenario=safe_scenario, messages=[DemoConversationMessage(**e) for e in messages]
    )
...
async def generate_assistant_suggestions(
    ...
    return AssistantSuggestionResponse(
        context=context, suggestions=suggestions, generated_by=generated_by
    )
