...
def _build_methodology_editor_payload(methodology_id: str) -> MethodologyEditorPayload:
    ...
    for index, stage in enumerate(methodology_map.get("stages", [])):
        ...
        activities = [
            MethodologyActivityEditor(
                ...
                category=a.get("category", "methodology"),
    ...
    return MethodologyEditorPayload(
        methodology_id=methodology_id, stages=stages, gates=gates_payload
    )
...
def _validate_methodology_prereqs(stages: list[MethodologyStageEditor]) -> None:
    ...
    invalid = [
        ...
        if p not in activity_ids
    ]
    if invalid:
        raise HTTPException(
            status_code=422, detail="Invalid prerequisites: " + ", ".join(sorted(invalid))
        )
...
async def run_methodology_node_action(
    ...
    resolution_contract = resolve_runtime(
        methodology_id, stage_id, activity_id, task_id, lifecycle_event
    )
        ...
        stage_activities = [
            {
                ...
                "assistant_prompts": activity_lookup.get(a.id, {}).get("assistant_prompts", []),
                "prerequisites": a.prerequisites,
                "category": a.category or "methodology",
                "recommended_canvas_tab": a.recommended_canvas_tab or "document",
            }
...
async def get_methodology_runtime_actions(
    methodology_id: str, stage_id: str, activity_id: str | None = None, task_id: str | None = None
) -> dict[str, Any]:
    return {
        "methodology_id": methodology_id,
        "stage_id": stage_id,
        "activity_id": activity_id,
        "task_id": task_id,
        "actions": list_runtime_actions_for_node(methodology_id, stage_id, activity_id, task_id),
    }
...
async def run_workspace_methodology_node_action(
    workspace_id: str, payload: MethodologyNodeActionRequest
) -> MethodologyNodeActionResponse:
    result = await run_methodology_node_action(
        workspace_id,
        payload.methodology_id,
...
async def set_methodology_org_policy(request: Request) -> dict[str, Any]:
    ...
    get_audit_log_store().record_event(
        build_event(
            ...
            actor_type="user",
            roles=["admin"],
            action="methodology.policy.updated",
            resource_type="methodology_policy",
