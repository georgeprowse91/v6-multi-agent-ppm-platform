...
async def get_agent_profile(agent_id: str, request: Request) -> AgentProfileResponse:
    ...
    for mapping in mappings:
        ...
        template_rows[mapping.template_id] = AgentProfileTemplateMapping(
            ...
            methodology_nodes=[
                {
                    "methodology_id": n.methodology_id,
                    "stage_id": n.stage_id,
                    "activity_id": n.activity_id,
                    "task_id": n.task_id,
                }
                for n in mapping.methodology_bindings
            ],
        )
        for endpoint in [
    ...
    for rm in runtime_registry.mappings:
        ...
        methodology_nodes.add(
            (
                rm.key.methodology_id,
                rm.key.stage_id,
                rm.key.activity_id,
                rm.key.task_id,
                rm.key.lifecycle_event,
            )
        )
    ...
    return AgentProfileResponse(
        ...
        connectors_used=sorted(
            connector_rows.values(), key=lambda i: f"{i['connector_type']}::{i['system']}"
        ),
        methodology_nodes_supported=[
            ...
            for m, s, a, t, e in sorted(methodology_nodes)
        ],
...
async def run_agent_preview(
    ...
    matching = next(
        (
            m
            for m in runtime_registry.mappings
            if agent_id in m.resolution.agent_workflow.agent_ids
            and m.key.lifecycle_event in {"generate", "update", "review", "approve", "publish"}
        ),
        None,
    )
    if not matching:
    ...
    activity_id = (
        payload.activity_id if payload.activity_id is not None else matching.key.activity_id
    )
...
async def get_project_agent_settings(project_id: str, request: Request) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=200,
        content={"project_id": project_id, "tenant_id": tenant_id, "agents": response_agents},
    )


@router.patch("/api/agent-gallery/{project_id}/agents/{agent_id}")
async def update_project_agent_settings(
    project_id: str, agent_id: str, payload: AgentConfigUpdate, request: Request
) -> JSONResponse:
    session = _require_session(request)
    tenant_id = _tenant_id_from_request(request, session)
    if not tenant_id:
    ...
    try:
        updated = agent_settings_store.update_agent_settings(
            tenant_id, project_id, agent_id, enabled=payload.enabled, config=payload.config
        )
