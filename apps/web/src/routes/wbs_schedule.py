...
def _mock_dependency_map(program_id: str) -> DependencyMapResponse:
    return DependencyMapResponse(
        ...
        nodes=[
            DependencyNode(
                id="proj-alpha", label="Project Alpha", type="project", status="in_progress"
            ),
...
async def get_wbs(project_id: str, request: Request) -> WbsResponse:
    ...
    return WbsResponse(
        project_id=project_id, updated_at=datetime.now(timezone.utc).isoformat(), items=items
    )
...
async def get_schedule(project_id: str, request: Request) -> ScheduleResponse:
    ...
    return ScheduleResponse(
        project_id=project_id, updated_at=datetime.now(timezone.utc).isoformat(), tasks=tasks
    )
...
async def optimize_schedule(project_id: str, request: Request) -> OptimizeScheduleResponse:
    ...
    return OptimizeScheduleResponse(
        ...
        optimized_duration_days=max(
            total_dur - sum(s.projected_saving_days for s in suggestions), 0
        ),
...
@router.post(
    "/api/schedule/{project_id}/apply-optimization", response_model=ApplyOptimizationResponse
)
        ...
        if payload.suggestion_id == "opt-crash":
            for t in tasks:
                if t.start and t.end:
                    ...
                    new_end = (
                        _date.fromisoformat(t.start[:10]) + timedelta(days=crashed)
                    ).isoformat()
                    t.end = new_end
            ...
            for t in tasks:
                if t.dependencies:
                    ...
                    if pred and pred.end:
                        ...
                        new_start = (
                            _date.fromisoformat(pred.end[:10]) - timedelta(days=overlap)
                        ).isoformat()
