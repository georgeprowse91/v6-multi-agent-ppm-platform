...
def _build_activity_summary(
    ...
    children = [
        _build_activity_summary(child, state, completed) for child in activity.get("children", [])
    ]
...
def _build_workspace_response(state: WorkspaceState) -> WorkspaceStateResponse:
    ...
    for stage in methodology_map.get("stages", []):
        ...
        activities = [
            _build_activity_summary(a, state, completed) for a in stage.get("activities", [])
        ]
        stages.append(
            StageSummary(
                id=stage.get("id", ""),
    ...
    monitoring = [
        _build_activity_summary(a, state, completed) for a in methodology_map.get("monitoring", [])
    ]
    ...
    if state.current_activity_id:
        all_activities = [
            a for s in methodology_map.get("stages", []) for a in s.get("activities", [])
        ]
        current_activity = next(
            (a for a in all_activities if a.get("id") == state.current_activity_id), None
        )
        if current_activity:
    ...
    gating = GatingSummary(
        current_activity_access=ActivityAccessSummary(
            allowed=gating_access["allowed"],
            reasons=gating_access.get("reasons", []),
            missing_prereqs=gating_access.get("missing_prereqs", []),
        ),
    ...
    if state.current_activity_id and state.current_stage_id:
        ...
        if act:
            selected = SelectedActivitySummary(
                ...
                connector_id=act.get("connector_id"),
            )
        templates_here = list_templates_for_methodology_node(
            state.methodology or "hybrid",
        ...
        templates_required = [
            t for t in templates_here if t.lifecycle_events and "generate" in t.lifecycle_events
        ]
        templates_review = [
            t for t in templates_here if t.lifecycle_events and "review" in t.lifecycle_events
        ]
        runtime_actions = list_runtime_actions_for_node(
            state.methodology or "hybrid",
            state.current_stage_id,
            state.current_activity_id,
            task_id=None,
    ...
    return WorkspaceStateResponse(
        ...
        available_methodologies=list(available_methodologies()),
        methodology_map_summary=methodology_summary,
        gating=gating,
        selected_activity=selected,
        templates_available_here=templates_here,
        templates_required_here=templates_required,
        templates_in_review=templates_review,
        runtime_actions_available=runtime_actions,
...
async def get_workspace_state(
    project_id: str, request: Request, methodology: str | None = Query(default=None)
) -> WorkspaceStateResponse:
    ...
    if methodology is not None:
        ...
        state = workspace_state_store.update_selection(
            tenant_id,
            project_id,
            {"methodology": selected, "current_stage_id": None, "current_activity_id": None},
        )
