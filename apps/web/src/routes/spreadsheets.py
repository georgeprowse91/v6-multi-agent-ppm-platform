...
async def create_spreadsheet_sheet(
    ...
    logger.info(
        ...
        extra={"tenant_id": tenant_id, "project_id": project_id, "sheet_id": sheet.sheet_id},
    )
    return sheet
...
async def get_spreadsheet_sheet(project_id: str, sheet_id: str, request: Request) -> SheetDetail:
    ...
    logger.info(
        ...
        extra={"tenant_id": tenant_id, "project_id": project_id, "sheet_id": sheet_id},
    )
    return detail
...
async def update_spreadsheet_row(
    project_id: str, sheet_id: str, row_id: str, payload: RowUpdate, request: Request
) -> Row:
    session = _require_session(request)
    tenant_id = session["tenant_id"]
    try:
...
@router.delete(
    "/api/spreadsheets/{project_id}/sheets/{sheet_id}/rows/{row_id}", response_model=DeleteResult
)
...
@router.post(
    "/api/spreadsheets/{project_id}/sheets/{sheet_id}/import.csv", response_model=ImportResult
)
