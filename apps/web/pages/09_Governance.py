...
st.set_page_config(page_title="Project Lifecycle & Governance", layout="wide")

store = get_store()
user = sidebar_login(store)

st.title("Project Lifecycle & Governance")

# Requirements
payload = load_requirements()
domain_reqs = []
for d in payload.get("functional_domains", []):
    if d.get("title") == "6. Project Lifecycle & Governance":
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_9")
        if st.button("Run agent 9", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=9, actor=user.name, entity_id=selected_id, inputs=inputs)
