...
st.set_page_config(page_title="Program Management", layout="wide")
...
for d in payload.get("functional_domains", []):
    if d.get("title") == "4. Program Management":
...
with left:
    ...
    created_id = entity_create_form(store, user=user, entity_type="program")
    st.markdown("#### Select existing")
    selected_id = entity_picker(store, user=user, entity_type="program", label="program record")
    if created_id:
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_7")
        if st.button("Run agent 7", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=7, actor=user.name, entity_id=selected_id, inputs=inputs)
