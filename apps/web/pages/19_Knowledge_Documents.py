...
st.title("Knowledge & Document Management")

# Requirements
payload = load_requirements()
domain_reqs = []
for d in payload.get("functional_domains", []):
    if d.get("title") == "16. Knowledge & Document Management":
...
with left:
    ...
    created_id = entity_create_form(store, user=user, entity_type="knowledge")
    st.markdown("#### Select existing")
    selected_id = entity_picker(store, user=user, entity_type="knowledge", label="knowledge record")
    if created_id:
