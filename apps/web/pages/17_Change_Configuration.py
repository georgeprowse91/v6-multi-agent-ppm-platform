...
st.set_page_config(page_title="Change & Configuration Management", layout="wide")

store = get_store()
user = sidebar_login(store)

st.title("Change & Configuration Management")

# Requirements
payload = load_requirements()
domain_reqs = []
for d in payload.get("functional_domains", []):
    if d.get("title") == "14. Change & Configuration Management":
...
with left:
    ...
    created_id = entity_create_form(store, user=user, entity_type="change_request")
    st.markdown("#### Select existing")
    selected_id = entity_picker(store, user=user, entity_type="change_request", label="change_request record")
    if created_id:
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_17")
        if st.button("Run agent 17", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=17, actor=user.name, entity_id=selected_id, inputs=inputs)
