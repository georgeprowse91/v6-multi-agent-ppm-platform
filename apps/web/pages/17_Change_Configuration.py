...
with left:
    ...
    selected_id = entity_picker(
        store, user=user, entity_type="change_request", label="change_request record"
    )
    if created_id:
    ...
    if not selected_id:
        ...
        if st.button("Run agent 17", type="primary"):
            try:
                ...
                out = run_agent(
                    store, agent_id=17, actor=user.name, entity_id=selected_id, inputs=inputs
                )
