...
st.set_page_config(page_title="Release & Deployment Management", layout="wide")

store = get_store()
user = sidebar_login(store)

st.title("Release & Deployment Management")

# Requirements
payload = load_requirements()
domain_reqs = []
for d in payload.get("functional_domains", []):
    if d.get("title") == "15. Release & Deployment Management":
...
with left:
    ...
    created_id = entity_create_form(store, user=user, entity_type="release")
    st.markdown("#### Select existing")
    selected_id = entity_picker(store, user=user, entity_type="release", label="release record")
    if created_id:
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_18")
        if st.button("Run agent 18", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=18, actor=user.name, entity_id=selected_id, inputs=inputs)
