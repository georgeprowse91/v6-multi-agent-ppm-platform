...
st.set_page_config(page_title="Project Definition & Scope Management", layout="wide")

store = get_store()
user = sidebar_login(store)

st.title("Project Definition & Scope Management")

# Requirements
payload = load_requirements()
domain_reqs = []
for d in payload.get("functional_domains", []):
    if d.get("title") == "5. Project Definition & Scope Management":
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_8")
        if st.button("Run agent 8", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=8, actor=user.name, entity_id=selected_id, inputs=inputs)
