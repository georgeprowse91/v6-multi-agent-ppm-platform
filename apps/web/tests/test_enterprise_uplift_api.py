...
def test_scenario_results_deterministic(client):
    ...
    first = client.post(
        "/v1/api/portfolio/demo-portfolio/scenarios/run", json={"name": "Det", "limit": 5}
    ).json()
    second = client.post(
        "/v1/api/portfolio/demo-portfolio/scenarios/run", json={"name": "Det", "limit": 5}
    ).json()
...
def test_mention_creates_notification_and_mark_read(client):
    comment = client.post(
        ...
        json={
            ...
            "artifact_id": "a1",
            "text": "Please review @demo-user",
        },
    )
    assert comment.status_code == 200
    notifications = client.get("/v1/api/notifications").json()["notifications"]
    mention = next(
        item for item in notifications if "Mentioned in comment" in str(item.get("message", ""))
    )
...
def test_budget_versioning_and_diff(client):
    first = client.post(
        "/v1/api/finance/budgets", json={"workspace_id": "demo-program", "amounts": {"capex": 100}}
    ).json()
    second = client.post(
        "/v1/api/finance/budgets", json={"workspace_id": "demo-program", "amounts": {"capex": 130}}
    ).json()
...
def test_sync_diff_and_conflict_resolution(client):
    diff = client.post(
        "/v1/api/sync/diff",
        json={
            "source": [{"id": "x1", "name": "incoming"}],
            "target": [{"id": "x1", "name": "current"}],
        },
    ).json()
    assert diff["conflicts"]
    conflict_id = diff["conflicts"][0]["id"]
    resolved = client.post(
        f"/v1/api/sync/conflicts/{conflict_id}/resolve", json={"decision": "incoming"}
    ).json()
