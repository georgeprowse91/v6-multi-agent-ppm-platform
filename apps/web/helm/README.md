...
## Purpose
This chart packages the **web** deployment for the Multi-Agent PPM platform when deploying to
Kubernetes. It defines the Deployment, Service, and ConfigMap used by the web runtime.

## Responsibilities
- Package the web container image and runtime settings.
...
## Conventions
- `Chart.yaml` **name** must match the parent directory name (`web`).
...
1. Add a new manifest under `apps/web/helm/templates/` (e.g. `ingress.yaml`).
...
```bash
python scripts/validate-helm-charts.py apps/web/helm
```
...
```yaml
image:
  repository: ghcr.io/georgeprowse91/web
  tag: "0.1.0"
service:
  port: 8080
```
