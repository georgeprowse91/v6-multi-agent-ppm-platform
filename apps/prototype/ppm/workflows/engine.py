...
def start_workflow(store: Store, *, def_id: str, entity_id: str, actor: str) -> str:
    ...
    instance_id = store.create_workflow_instance(def_id=def_id, entity_id=entity_id, status="InProgress", current_step_id=first_step_id, context=context)
    store.log_event(actor=actor, event_type="workflow_started", entity_id=entity_id, details={"instance_id": instance_id, "def_id": def_id})
    return instance_id
...
def advance_workflow(
    ...
    *,
    ...
    result: Dict[str, Any] = {"instance_id": instance_id, "step_id": current_step_id, "step_name": step.get("name")}

    if step.get("terminal"):
        store.update_workflow_instance(instance_id, status="Completed", current_step_id=None, context={**ctx, "history": history})
        return {**result, "status": "Completed"}

    if step.get("gate"):
        if approve is None:
            return {**result, "status": "AwaitingApproval", "message": "This step is an approval gate; provide approve=True/False."}
        ...
        history.append({"step_id": current_step_id, "type": "gate", "approved": approve, "actor": actor, "timestamp": now_iso()})
        store.update_workflow_instance(instance_id, current_step_id=next_step_id, context={**ctx, "history": history})
        store.log_event(actor=actor, event_type="workflow_gate_decision", entity_id=inst["entity_id"], details={"instance_id": instance_id, "step_id": current_step_id, "approved": approve})
    ...
    if agent_id and agent_id not in (1, 2, 3):
        agent_out = run_agent(store, agent_id=agent_id, actor=actor, entity_id=inst["entity_id"], inputs=inputs)

    next_list = step.get("next") or []
    next_step_id = next_list[0] if isinstance(next_list, list) and next_list else None

    history.append({"step_id": current_step_id, "type": "agent", "agent_id": agent_id, "actor": actor, "timestamp": now_iso(), "outputs": agent_out})
    store.update_workflow_instance(instance_id, current_step_id=next_step_id, context={**ctx, "history": history})
    store.log_event(actor=actor, event_type="workflow_step_completed", entity_id=inst["entity_id"], details={"instance_id": instance_id, "step_id": current_step_id, "agent_id": agent_id})
