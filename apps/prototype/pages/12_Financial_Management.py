...
st.set_page_config(page_title="Financial Management", layout="wide")
...
for d in payload.get("functional_domains", []):
    if d.get("title") == "9. Financial Management":
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_12")
        if st.button("Run agent 12", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=12, actor=user.name, entity_id=selected_id, inputs=inputs)
