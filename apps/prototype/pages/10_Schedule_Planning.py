...
st.set_page_config(page_title="Schedule & Planning", layout="wide")
...
for d in payload.get("functional_domains", []):
    if d.get("title") == "7. Schedule & Planning":
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_10")
        if st.button("Run agent 10", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=10, actor=user.name, entity_id=selected_id, inputs=inputs)
