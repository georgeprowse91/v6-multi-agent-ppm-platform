...
st.set_page_config(page_title="Vendor & Procurement Management", layout="wide")

store = get_store()
user = sidebar_login(store)

st.title("Vendor & Procurement Management")

# Requirements
payload = load_requirements()
domain_reqs = []
for d in payload.get("functional_domains", []):
    if d.get("title") == "10. Vendor & Procurement Management":
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_13")
        if st.button("Run agent 13", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=13, actor=user.name, entity_id=selected_id, inputs=inputs)
