...
st.set_page_config(page_title="Risk & Issue Management", layout="wide")

store = get_store()
user = sidebar_login(store)

st.title("Risk & Issue Management")

# Requirements
payload = load_requirements()
domain_reqs = []
for d in payload.get("functional_domains", []):
    if d.get("title") == "12. Risk & Issue Management":
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_15")
        if st.button("Run agent 15", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=15, actor=user.name, entity_id=selected_id, inputs=inputs)
