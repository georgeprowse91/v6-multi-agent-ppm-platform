...
st.set_page_config(page_title="Communications & Stakeholder Management", layout="wide")

store = get_store()
user = sidebar_login(store)

st.title("Communications & Stakeholder Management")

# Requirements
payload = load_requirements()
domain_reqs = []
for d in payload.get("functional_domains", []):
    if d.get("title") == "18. Communications & Stakeholder Management":
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_21")
        if st.button("Run agent 21", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=21, actor=user.name, entity_id=selected_id, inputs=inputs)
