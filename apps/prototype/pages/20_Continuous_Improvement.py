...
st.set_page_config(page_title="Continuous Improvement & Process Mining", layout="wide")

store = get_store()
user = sidebar_login(store)

st.title("Continuous Improvement & Process Mining")

# Requirements
payload = load_requirements()
domain_reqs = []
for d in payload.get("functional_domains", []):
    if d.get("title") == "17. Continuous Improvement & Process Mining":
...
with left:
    ...
    created_id = entity_create_form(store, user=user, entity_type="project")
    st.markdown("#### Select existing")
    selected_id = entity_picker(store, user=user, entity_type="project", label="project record")
    if created_id:
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_20")
        if st.button("Run agent 20", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=20, actor=user.name, entity_id=selected_id, inputs=inputs)
