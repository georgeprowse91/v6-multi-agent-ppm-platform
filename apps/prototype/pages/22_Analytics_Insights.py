...
for d in payload.get("functional_domains", []):
    if d.get("title") == "19. Analytics & Insights":
...
with right:
    ...
    if not selected_id:
        ...
        inputs_text = st.text_area("Agent inputs (JSON)", value="{}", height=120, key="inputs_22")
        if st.button("Run agent 22", type="primary"):
            try:
                inputs = json.loads(inputs_text) if inputs_text.strip() else {}
                out = run_agent(store, agent_id=22, actor=user.name, entity_id=selected_id, inputs=inputs)
