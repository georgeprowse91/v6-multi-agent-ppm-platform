...
def sync_outbound(request: OutboundSyncRequest) -> dict[str, object]:
    ...
    try:
        rate_limit = 120
        config = WorkdayConfig.from_env(rate_limit)
        token_manager = _build_token_manager(config)
        client = _build_client(config, token_manager)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except HttpClientError as exc:
        raise _http_error("Failed to initialize Workday outbound sync", exc) from exc

    endpoint = os.getenv("WORKDAY_OUTBOUND_ENDPOINT") or "/projects"
