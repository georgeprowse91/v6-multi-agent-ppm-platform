"""Workday MCP mapping helpers.

Operations
----------
list
    Required fields:
        - resource_type (str): Expected enum: "workers" | "positions".
    Optional fields:
...
def map_to_mcp_params(operation: str, canonical_payload: dict[str, Any]) -> dict[str, Any]:
    """Map canonical payloads to Workday MCP tool parameters."""
    if operation != "list":
        raise ValueError(f"Unsupported Workday MCP operation: {operation}")

    return {
        "resource_type": canonical_payload.get("resource_type", "workers"),
...
def map_from_mcp_response(operation: str, tool_response: Any) -> list[dict[str, Any]]:
    """Normalize MCP tool responses into canonical Workday payloads."""
    if operation != "list":
        raise ValueError(f"Unsupported Workday MCP operation: {operation}")
