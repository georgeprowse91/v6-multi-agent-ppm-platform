...
router = APIRouter(prefix="/connectors/notification_hubs", tags=["connectors"])


def _build_connector() -> NotificationHubsConnector:
    ...
    return NotificationHubsConnector(config)
...
def sync_inbound(request: InboundSyncRequest) -> list[dict[str, object]]:
    ...
    notifications = connector.read("notifications")
    records = [
        {
            "source": "project",
            "id": notification.get("id"),
            "name": notification.get("title"),
            "status": "sent",
            "owner": notification.get("to"),
        }
...
def sync_outbound(request: OutboundSyncRequest) -> dict[str, object]:
    ...
    if request.live:
        connector = _build_connector()
        response = connector.write("notifications", request.records)
