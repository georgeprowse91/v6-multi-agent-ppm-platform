"""Microsoft Teams MCP mapping helpers.

Operations
----------
list
    Required fields:
        - resource_type (str): Expected enum: "teams" | "channels".
    Optional fields:
...
write
    ...
    Optional fields:
        - team_id (str): override team identifier for MCP tools that accept it.
        - channel_id (str): override channel identifier for MCP tools that accept it.
    Type conversions:
...
def map_to_mcp_params(operation: str, canonical_payload: dict[str, Any]) -> dict[str, Any]:
    """Map canonical payloads to Teams MCP tool parameters."""
    resource_type = canonical_payload.get("resource_type")

    if operation == "list":
        return {
            "resource_type": resource_type or "teams",
    ...
    if operation == "write":
        ...
        if canonical_payload.get("team_id") is not None:
            payload["team_id"] = canonical_payload["team_id"]
        if canonical_payload.get("channel_id") is not None:
            payload["channel_id"] = canonical_payload["channel_id"]
        return payload

    raise ValueError(f"Unsupported Teams MCP operation: {operation}")


def map_from_mcp_response(operation: str, tool_response: Any) -> list[dict[str, Any]]:
    """Normalize MCP tool responses into canonical Teams payloads."""
    if operation == "write":
