"""
Tests for the Clarity Connector.
"""
...
CLARITY_CONNECTOR_PATH = REPO_ROOT / "integrations" / "connectors" / "clarity" / "src"
...
def mock_env():
    env_vars = {
        "CLARITY_INSTANCE_URL": "https://clarity.example.com",
        "CLARITY_CLIENT_ID": "client-id",
        "CLARITY_CLIENT_SECRET": "client-secret",
        "CLARITY_REFRESH_TOKEN": "refresh-token",
    }
...
def test_test_connection_success(mock_env, clarity_config):
    ...
    connector = ClarityConnector(clarity_config, transport=transport, token_manager=token_manager)
...
def test_read_projects_success(mock_env, clarity_config):
    transport = MockTransport(
        {
            "/ppm/rest/v1/projects": [
                {"status_code": 200, "content": {"items": [], "total": 0, "offset": 0}},
    ...
    assert projects[0]["id"] == "clarity-1"
    assert projects[0]["program_id"] == "portfolio-1"
