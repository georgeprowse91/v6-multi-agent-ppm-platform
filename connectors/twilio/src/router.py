...
router = APIRouter(prefix="/connectors/twilio", tags=["connectors"])


def _build_connector() -> TwilioConnector:
...
def sync_inbound(request: InboundSyncRequest) -> list[dict[str, object]]:
    ...
    messages = connector.read("messages")
    records = [
        {
            "source": "project",
            "id": message.get("sid"),
