...
class DataSyncAgent(BaseAgent):
    ...
    def _load_quality_thresholds(self) -> dict[str, float | dict[str, float]]:
        thresholds_path = (
            Path(
                self.config.get(
                    "quality_thresholds_path", "ops/config/agents/data-synchronisation-agent/quality_thresholds.yaml"
                )
            )
            if self.config
    ...
    async def governed_connector_write(
        ...
        *,
        ...
        """
        connector = self.connectors.get(connector_id) if hasattr(self, "connectors") else None
        connector_status = "connected" if connector is not None else "not_configured"

        gate_result = self.write_gate.check(
            connector_status=connector_status,
            approval_required=approval_required,
            approval_status=approval_status,
            dry_run_passed=True if not self.write_gate.require_dry_run else (not dry_run),
