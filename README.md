...
The web console is a React 18 single-page application built with Vite, Zustand for state management, and React Router 6. It delivers a three-panel shell layout with an AI assistant embedded alongside every workspace view.

### Shell Layout

Every authenticated page renders inside `AppLayout`, which composes:

- **Header** --- Top application bar with platform wordmark, global search, user avatar, and notification badge.
- **LeftPanel** --- Collapsible navigation sidebar that switches between hub navigation (portfolios, programs, projects, admin) and project-workspace navigation (methodology tree, project-scoped tools) depending on the active route.
- **MainCanvas** --- Scrollable content region hosting the active page component.
- **AssistantPanel** --- Persistent right-hand conversational AI assistant with context-aware suggestions, action chips, LLM model selection, and demo mode scripted scenarios.

### Key Page Surfaces

| Surface | Route | Description |
|---------|-------|-------------|
| Home | `/` | Entry view with KPI summary cards and demo project launchers |
| Entity Workspaces | `/portfolio/:id`, `/program/:id`, `/project/:id` | Unified workspace with pipeline board, KPI widgets, and drag-and-drop management |
| Methodology Workspace | Project routes | Stage/activity map canvas, activity detail panel, lifecycle actions (generate, review, approve, publish) |
| Intake Flow | `/intake/new`, `/intake/status/:id`, `/intake/approvals` | Multi-step project intake with concept, details, attachments, and approval routing |
| Analytics Dashboard | `/analytics/dashboard` | Trend charts, predictive alert badges, and KPI tables |
| Connector Marketplace | `/marketplace/connectors` | Paginated connector registry with category filter, enable/disable, and connection testing |
| Agent Gallery | `/ops/config/agents` | Agent listing, filtering, and configuration |
| Workflow Tools | `/workflows/monitoring`, `/workflows/designer` | Live workflow run monitoring and visual workflow authoring |
| Approval Queues | `/approvals` | General and intake approval processing with approve/reject decisions |
| Admin Pages | `/admin/audit`, `/admin/roles`, `/admin/methodology`, `/admin/agent-runs` | Audit logs, role management, methodology editing, and agent run history (permission-gated) |
| Knowledge | `/knowledge/documents`, `/knowledge/lessons` | Document search and lessons learned repository |

### Assistant Panel Modes

The AI assistant adapts its behaviour based on the current context:

| Mode | Context | Behaviour |
|------|---------|-----------|
| Entry | Home and non-workspace routes | Quick-action launcher chips |
| Intake | `/intake/new` | Routes messages through the intake adapter; patches apply directly to the intake form |
| Workspace | `/projects/:id` | Context-aware methodology suggestions generated by the suggestion engine |
| Demo | `VITE_DEMO_MODE=true` | Replays scripted conversation scenarios without live LLM calls |

### Canvas Artifact Types

The methodology workspace supports 13 canvas artifact types, each rendered in a purpose-built editor:

Document, Structured Tree (WBS), Timeline, Gantt, Board (Kanban), Backlog, Grid, Spreadsheet, Financial, Dashboard, Dependency Map, Roadmap, and Approval.

Artifacts carry provenance metadata (`sourceAgent`, `generatedAt`, `correlationId`) and move through a `draft` to `published` lifecycle with formal approval workflows.

### Auth Guards and Feature Flags

All routes are protected by `RequireAuth` and `RequireTenantContext` guards. Admin pages additionally require `RequireAdminRole`. Several surfaces are gated behind feature flags:

| Flag | Surfaces |
|------|----------|
| `duplicate_resolution` | Merge review page |
| `agent_async_notifications` | Notification center |
| `agent_run_ui` | Agent runs visibility in navigation |
| `predictive_alerts` | Analytics predictive alerts |
| `multimodal_intake` | Intake upload and extract endpoints |

### Streamlit Demo

A standalone Streamlit demo (`apps/demo_streamlit/app.py`) mirrors the web console for offline and sales demos. It requires no backend services, loads JSON fixtures for all data, and writes all actions to an append-only outbox file. It includes panels for Home, Workspace, Dashboard, Approvals, Intake, Collections, Agent Gallery, Analytics What-If, Connector Registry, Audit, Notifications, Demo Run, and Agent Runs.

### Accessibility

- Skip-to-content anchor as the first focusable element
- Keyboard navigation (Arrow Up/Down, Home, End) across all nav items
- `aria-label` and `title` attributes on collapsed icon-only sidebar links
- Semantic icon system with `decorative` and `label` props enforcing accessible usage

For the full component-by-component reference, coverage matrix, and known gap backlog, see [docs/UI.md](docs/UI.md).
