# Business Case

## Purpose
Provide a structured, executive-ready rationale for investment, including strategic alignment,
options analysis, financial outcomes, risk exposure, and implementation readiness.

## Audience
Executive Sponsor, Portfolio Manager, Finance Partner, PMO Lead.

## Inputs
- Demand intake request (`docs/templates/portfolio-program/demand-intake-request-template-cross.md`).
- Strategic objectives and portfolio constraints.
- Cost estimates, benefit assumptions, and baseline performance metrics.
- Risk register and dependency inventory.

## Definitions
- **Net Present Value (NPV):** Present value of cash inflows minus outflows.
- **Internal Rate of Return (IRR):** Discount rate that makes NPV equal to zero.
- **Return on Investment (ROI):** Percentage benefit relative to cost.
- **Payback Period:** Time required to recover the initial investment.

## Required Sections
- Executive summary and recommendation
- Problem statement and opportunity
- Strategic alignment and success criteria
- Options analysis and comparison
- Financial analysis and sensitivity
- Risk and dependency assessment
- Implementation approach and governance
- Approvals and document control

---

## Document Information
- **Project name:**
- **Business case owner:**
- **Executive sponsor:**
- **Version:**
- **Date prepared:**
- **Document classification:** [Confidential/Internal/Public]
- **Approval required:** [Yes/No]

## 1. Executive Summary
### 1.1 Project Overview
- **Project sponsor:**
- **Business case owner:**
- **Target start/end dates:**
- **Executive approval required:**

### 1.2 Investment Summary
| Metric | Value | Target/Benchmark |
| --- | --- | --- |
| Total investment | $ | $ |
| Expected ROI | % | % |
| Payback period |  |  |
| Net present value (NPV) | $ | $0 |
| Internal rate of return (IRR) | % | Cost of capital % |

### 1.3 Strategic Justification
Summarize the strategic rationale and expected enterprise impact in 2–3 paragraphs.

### 1.4 Executive Recommendation
**Decision:** [Approve/Defer/Reject]

**Rationale:**
- 
- 

## 2. Problem Statement & Opportunity
### 2.1 Business Problem
Describe the core pain point, root causes, and the urgency of the problem.

### 2.2 Current State Analysis
- **Current process/system:**
- **Key pain points:**
  - 
  - 
  - 
- **Quantified impact:**
  - Annual cost of problem: $
  - Productivity loss: %
  - Customer impact:

### 2.3 Market Opportunity (if applicable)
- **Market size:** $
- **Target market share:** %
- **Revenue opportunity:** $
- **Competitive advantage:**

### 2.4 Drivers for Change
**Internal drivers**
- 
- 

**External drivers**
- 
- 

## 3. Strategic Alignment
### 3.1 Organizational Strategy Alignment
| Strategic Objective | Alignment Narrative | Metric | Owner |
| --- | --- | --- | --- |
|  |  |  |  |

### 3.2 Success Criteria
| Success Criterion | Target Metric | Measurement Method | Responsible Party |
| --- | --- | --- | --- |
|  |  |  |  |

## 4. Solution Overview
### 4.1 Proposed Solution
Describe the proposed solution at a high level, including business outcomes.

### 4.2 Key Features and Capabilities
1. **Capability:**
   - Description:
   - Business value:

2. **Capability:**
   - Description:
   - Business value:

### 4.3 Solution Architecture (High-Level)
Summarize the architecture, integration points, and security requirements.

### 4.4 Technology Components
- **Platform:**
- **Integration requirements:**
- **Infrastructure needs:**
- **Security/compliance requirements:**

## 5. Options Analysis
### 5.1 Options Considered
| Option | Description | Benefits | Costs | Risks | Recommendation |
| --- | --- | --- | --- | --- | --- |
| Do nothing |  |  |  |  |  |
| Proposed solution |  |  |  |  |  |
| Alternative solution |  |  |  |  |  |

### 5.2 Options Comparison Matrix
| Criteria | Weight | Option 1 | Option 2 | Option 3 |
| --- | --- | --- | --- | --- |
| Cost | % |  |  |  |
| Benefit realization | % |  |  |  |
| Risk level | % |  |  |  |
| Implementation complexity | % |  |  |  |
| Strategic fit | % |  |  |  |
| **Weighted total** | **100%** |  |  |  |

## 6. Financial Analysis
### 6.1 Investment Requirements
#### Capital Expenditure (CapEx)
| Category | Year 0 | Year 1 | Year 2 | Year 3 | Total |
| --- | --- | --- | --- | --- | --- |
| Hardware |  |  |  |  |  |
| Software licenses |  |  |  |  |  |
| Implementation services |  |  |  |  |  |
| Training |  |  |  |  |  |
| **Total CapEx** |  |  |  |  |  |

#### Operational Expenditure (OpEx)
| Category | Year 1 | Year 2 | Year 3 | Year 4 | Year 5 | Total |
| --- | --- | --- | --- | --- | --- | --- |
| Software maintenance |  |  |  |  |  |  |
| Support services |  |  |  |  |  |  |
| Infrastructure |  |  |  |  |  |  |
| Internal resources |  |  |  |  |  |  |
| **Total OpEx** |  |  |  |  |  |  |

### 6.2 Benefit Analysis
#### Financial Benefits
| Benefit Category | Year 1 | Year 2 | Year 3 | Year 4 | Year 5 | Total |
| --- | --- | --- | --- | --- | --- | --- |
| Cost savings |  |  |  |  |  |  |
| Revenue increase |  |  |  |  |  |  |
| Productivity gains |  |  |  |  |  |  |
| Cost avoidance |  |  |  |  |  |  |
| **Total benefits** |  |  |  |  |  |  |

#### Qualitative Benefits
- **Strategic benefits:**
- **Operational benefits:**
- **Risk mitigation benefits:**

### 6.3 Cash Flow & ROI Metrics
| Year | Investment | Benefits | Net Cash Flow | Cumulative Cash Flow |
| --- | --- | --- | --- | --- |
| 0 |  |  |  |  |
| 1 |  |  |  |  |
| 2 |  |  |  |  |
| 3 |  |  |  |  |
| 4 |  |  |  |  |
| 5 |  |  |  |  |

**Financial Metrics**
- **NPV:** $ (Discount rate: %)
- **IRR:** %
- **Payback period:**
- **ROI:** %
- **Benefit-cost ratio:** :1

Reference the ROI/NPV worksheet (`docs/templates/finance/roi-npv-worksheet-cross.xlsx`).

### 6.4 Sensitivity & Scenario Analysis
| Scenario | NPV | IRR | Payback Period | Comments |
| --- | --- | --- | --- | --- |
| Base case |  |  |  |  |
| Best case |  |  |  |  |
| Worst case |  |  |  |  |

## 7. Risk & Dependency Analysis
### 7.1 Risk Assessment Matrix
| Risk | Probability | Impact | Risk Level | Mitigation Strategy | Owner |
| --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |

### 7.2 Key Assumptions
| Assumption | Rationale | Impact if False | Validation Plan |
| --- | --- | --- | --- |
|  |  |  |  |

### 7.3 Critical Dependencies
| Dependency | Impact if Not Met | Mitigation | Owner |
| --- | --- | --- | --- |
|  |  |  |  |

## 8. Organizational Impact
### 8.1 Change Management Requirements
| Stakeholder Group | Impact Level | Change Required | Resistance Level | Engagement Strategy |
| --- | --- | --- | --- | --- |
|  |  |  |  |  |

### 8.2 Training Requirements
- **Training budget:** $
- **Training duration:**
- **Training methods:**
- **Users to train:**

### 8.3 Organizational Readiness
- **Change readiness score:** /10
- **Leadership support:**
- **Resource availability:**
- **Cultural fit:**

## 9. Implementation Approach
### 9.1 Delivery Phases
| Phase | Duration | Objectives | Key Deliverables | Success Criteria |
| --- | --- | --- | --- | --- |
| Requirements & analysis |  |  |  |  |
| Design |  |  |  |  |
| Implementation |  |  |  |  |
| Testing |  |  |  |  |
| Deployment |  |  |  |  |

### 9.2 Timeline & Milestones
- **Total duration:**
- **Start date:**
- **End date:**
- **Key milestones:**
  - 
  - 

### 9.3 Resource Requirements
- **Project manager:** % allocation
- **Business analyst:** % allocation
- **Technical team:** # resources
- **Subject matter experts:** % allocation
- **External vendors:**

## 10. Governance & Decision Points
### 10.1 Governance Structure
- **Steering committee:**
- **Project sponsor:**
- **Business case owner:**
- **Project manager:**

### 10.2 Decision Gates
| Gate | Date | Criteria | Approvers |
| --- | --- | --- | --- |
| Concept approval |  |  |  |
| Design approval |  |  |  |
| Implementation approval |  |  |  |
| Deployment approval |  |  |  |

### 10.3 Approval Readiness Checklist
- [ ] Strategic alignment confirmed.
- [ ] Financial case validated.
- [ ] Risk assessment completed.
- [ ] Resource availability confirmed.
- [ ] Stakeholder buy-in obtained.
- [ ] Implementation plan approved.

## 11. Recommendation & Next Steps
### 11.1 Final Recommendation

### 11.2 Rationale

### 11.3 Immediate Next Steps
1.
2.
3.

## 12. Appendices
- **Appendix A:** Detailed financial model
- **Appendix B:** Market research and analysis
- **Appendix C:** Technical architecture details
- **Appendix D:** Stakeholder impact analysis
- **Appendix E:** Risk register

---

## Document Control
| Version | Date | Author | Changes |
| --- | --- | --- | --- |
| 1.0 |  |  | Initial draft |

## Related Templates
- [ROI/NPV worksheet](./roi-npv-worksheet.xlsx)
- [Assumption log](./assumption-log.md)
- [Requirements specification](./requirements-specification.md)
- [Project management plan](./project-management-plan.md)
- [Project scope statement](./project-scope-statement.md)

## Business Case Maintenance Guidance
Revisit the business case at each major gate or when scope, schedule, or cost changes occur.
Update assumptions, financials, and dependencies to reflect approved changes.

## Completion Checklist
- [ ] Strategic alignment documented.
- [ ] Options evaluated with recommendation.
- [ ] Financial analysis validated by Finance.
- [ ] Risks, assumptions, and dependencies documented.
- [ ] Approval recorded.

## Acceptance Criteria
- Business case includes clear value, costs, and assumptions.
- Financial metrics calculated and reviewed.
- Decision-makers approve the recommended option.
