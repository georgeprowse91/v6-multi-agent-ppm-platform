...
Beyond these core examples, the platform also supports additional canvas styles used in specific workflows, including board/backlog views, Gantt and grid-style planning surfaces, financial views, dependency mapping, roadmap views, and approval-focused canvases.
...
The platform has been designed in alignment with enterprise security standards. Authentication is handled via OIDC and integrates with Azure Active Directory and Okta. All data in transit is encrypted, and all data at rest is encrypted using Azure-managed keys. Identity lifecycle and provisioning integrations should be confirmed per engagement environment and runbook. Its security architecture has been documented against applicable frameworks including the Australian Government's ISM and PSPF, and APRA CPS 234, making it suitable for deployment in financial services and public sector contexts.
...
| **Integrations** | Extensible connector framework spanning PPM, ERP, collaboration, GRC and communications systems |
...
## Current Collateral Inventory (Repository-Aligned)

Use the following **existing** collateral as the source set for sales messaging and deal support:

- `market-and-problem-analysis.md` for market narrative, problem framing, and buyer urgency.
- `competitive-positioning.md` for competitor comparisons and positioning statements.
- `go-to-market-plan.md` for segment priorities, channel model, and execution sequencing.
- `packaging-and-pricing.md` for commercial packaging options and pricing guidance.
- `../02-solution-design/platform-architecture-overview.md` and `../02-solution-design/agent-system-design.md` for architecture and agent capability substantiation.

### Messaging Guardrails

- Do not reference collateral files that are not present in the repository (for example standalone `solution_brief.md`, `demo_script.md`, or `roi_calculator.xlsx`).
- Do not make hard numeric connector claims (for example "38+ connectors") unless validated against the current connector catalog and release notes.
- Position deployment options as currently documented: managed SaaS and private-cloud options are primary; other models should be presented as roadmap/strategic depending on current GTM documentation.
- Validate implementation-sensitive claims (for example SCIM, immutable audit retention semantics, and exact integration breadth) against architecture and runbook documentation before external use.
