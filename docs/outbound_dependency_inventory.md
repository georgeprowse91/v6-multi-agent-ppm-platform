...
- `services/orchestration-service/src/workflow_client.py` calls workflow-service APIs.
- `services/workflow-service/src/agent_client.py` and runtime HTTP step execution in `services/workflow-service/src/workflow_runtime.py`.
...
- `services/api-gateway/src/api/middleware/security.py` contains OIDC/JWKS fetch logic.

## Connector APIs
- Connector SDK base call path in `connectors/sdk/src/base_connector.py` with connector implementations inheriting it.
- Web/API proxy connector routes in `services/api-gateway/src/api/routes/connectors.py`.

## Search and external research clients
- `services/api-gateway/src/api/routes/risk_research.py` delegates external risk research through agent paths.
...
- `services/analytics-service/src/main.py` + `services/analytics-service/src/kpi_engine.py`.
- `services/api-gateway/src/api/routes/analytics.py` + `services/api-gateway/src/api/routes/lineage.py`.
