...
| Auditability | Security-relevant flows must produce immutable audit evidence with retention enforcement and evidence export capability. | `services/audit-log/src/main.py`, `services/audit-log/src/audit_storage.py`, `data/schemas/audit-event.schema.json`, `ops/config/retention/policies.yaml` |
